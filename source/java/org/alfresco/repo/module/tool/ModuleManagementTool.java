...
public class ModuleManagementTool implements LogOutput
{
    ...
    public void exportModule(String moduleId,String warFileLocation, boolean preview, boolean purge) throws IOException
    {
        InstalledFiles installedFiles = new InstalledFiles(warFileLocation, moduleId);
        installedFiles.load();
        ...
        for (String add : installedFiles.getAdds())
        {
            addFile(moduleId, warFileLocation, add, preview);
        }
        // Extract the module properties
        String modulePropertiesFileLocationInWar = ModuleDetailsHelper.getModulePropertiesFilePathInWar(moduleId);
        addFile(moduleId, warFileLocation, modulePropertiesFileLocationInWar, preview);
    }
    
    private void addFile(String moduleId, String warLocation, String filePath, boolean preview) throws IOException
    {
    	TFile addFile = new TFile(warLocation + filePath);
        if (addFile.exists() == true)
        {
            outputVerboseMessage("Adding file '" + filePath + "' from war", true);
            if (preview == false)
            {
            	// Manual calculations, this path has to be mapped following Alfresco rules 
            	String ampFilePath = filePath.replace("/WEB-INF/", "/");
            	ampFilePath = ampFilePath.replace("/classes/", "/config/");
            	if (ampFilePath.endsWith("/module.properties")) ampFilePath="/module.properties";
            	
            	File entry = new TFile(moduleId + ".amp/" + ampFilePath);
            	TFile.cp_p(addFile, entry);
            }
        }
        else
        {
...
//                            zipController.reset();
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            else if (OP_EXPORT.equals(operation) == true)
            {
            	if (args.length < 3) 
            	{
            		throw new UsageException(OP_EXPORT + " requires at least 3 arguments.");
            	}
                String moduleId = args[1];
                String warLocation = args[2];
                boolean purge = false;
                ...
                manager.setVerbose(true);
                manager.exportModule(moduleId, warLocation,preview, purge);
                System.exit(SUCCESS_EXIT_CODE);
            }
            else
            {
    ...
    private static void outputUsage()
    {
        ...
        System.out.println("-----------------------------------------------------------\n");    
        System.out.println("export:  Exports a module from the Alfresco WAR file.");
        System.out.println("usage: export <ModuleId> <WARFileLocation>\n");
        System.out.println("-----------------------------------------------------------\n");    
    }
